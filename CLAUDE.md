...
- 最終更新: 2026-05-11（コメント欄・進捗バー・ドメイン変更）
...
- 手順: お名前.com のネームサーバーを Cloudflare に移管 → Cloudflare Pages にカスタムドメイン追加
- baseURL は `https://reslabo.jp` に変更済み ✅（コード側完了）
...
- AI記事自動生成パイプライン（generate-article.py）
...
### 優先度 高（ユーザー操作が必要）
- [ ] **DNS設定完了**：お名前.com のネームサーバーを Cloudflare に移管 → `reslabo.jp` でアクセス確認
- [ ] **Google Search Console 再設定**：ドメイン変更後に `reslabo.jp` でプロパティ追加・サイトマップ再送信
- [ ] **もしもアフィリエイト再申請**：DNS反映後に `reslabo.jp` で再申請
